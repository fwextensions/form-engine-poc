	...
	return (
		...
		>
			...
			{(!isViewMode && (showSubmit || showNext || showFinalSubmitMultiPage || showPrevious)) && (
				<div className={`mt-6 pt-5 border-t border-ink-200 flex ${showPrevious ? 'justify-between' : 'justify-end'} items-center ${config.buttonsClassName || ''}`}>
					{showPrevious && (
						<button
							type="button" // Important: not a submit type
							onClick={onNavigatePrev}
							className={`px-4 py-2 text-ink-700 bg-ink-50 rounded shadow-sm hover:bg-ink-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ink-400 disabled:opacity-50 ${config.previousButtonClassName || ''}`}
							disabled={isViewMode}
						>
					...
					{(showSubmit || showNext || showFinalSubmitMultiPage) && (
						<Submit asChild>
							<button
								type="submit"
								className={`px-4 py-2 bg-primary-600 text-white rounded shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed ${showNext ? (config.nextButtonClassName || '') : (config.submitButtonClassName || '')}`}
								disabled={isViewMode}
							>
